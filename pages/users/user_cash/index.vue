		...
		<view class='cash-withdrawal'>
			<view class='nav acea-row'>
				<view v-for="(item,index) in navList" :key="index" class='item font-color' @click="swichNav(index)">
					...
					<view style="color: #3E3E3E;">{{item.name}}</view>
				</view>
			</view>
			<view class="masklist">
				<view class="box" @click="getrecord">
					提现记录
				</view>
			</view>
				...
				<view :hidden='currentTab != 2' class='list'>
					<form @submit="subCash">
						...
						<view class='item acea-row row-top row-between'>
							<view class='name'>收款码</view>
							<view class="input acea-row">
								<u-upload max-count="1" :action="action" ref="uUploads"></u-upload>
							</view>
						</view>
						<view class='tip'>
							当前可提现金额: <text
						...
						<button formType="submit" :disabled="disabled" class='bnt bg-color'>提现</button>
					</form>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			uploadpic: function(type) {
				let that = this;
				that.$util.uploadImageOne('v3/upload', function(res) {
					if (type === 'W') {
			...
			getrecord() {
				uni.navigateTo({
					url: '../user_spread_money/index?type=' + 1
				})
			},
			/**
			 * 获取个人用户信息
				...
				if (that.currentTab == 0) { //银行卡
					...
					if (files.length > 0) {
						for (let i = 0; i < files.length; i++) {
							that.qrcodeUrlW += files[i].response + ',';
						}
						that.qrcodeUrlW = that.qrcodeUrlW.substr(0, that.qrcodeUrlW.length - 1);
					}
					...
					let files = [];
					files = that.$refs.uUploads.lists.filter(val => {
						return val.progress == 100;
					})
					if (files.length > 0) {
						for (let i = 0; i < files.length; i++) {
							that.qrcodeUrlZ += files[i].response + ',';
						}
						that.qrcodeUrlZ = that.qrcodeUrlZ.substr(0, that.qrcodeUrlZ.length - 1);
					}
