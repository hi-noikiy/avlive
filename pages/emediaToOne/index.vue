 <template>
	<view class="page">
	<view>
		<text style="color: #fff">音视频通话 {{time}}</text>
	</view>
	<view>
		<text style="color: #fff; font-size: 16rpx;">音视频通话id {{confrId}}</text>
	</view>
	<view class="videoBox">
		<view>
		<emlive-pusher 
...
import chatEmediaInvite from "@/comps/invite/index.nvue"
	import {APP_NAME,getSysUserInfo,CHAT_PREFIX} from '@/utils/appIM.js'
	export default {
		data() {
			return {
				livePlayes: [],
		...
		methods: {
			joinRoom(data){
				...
				wx.emedia.mgr.createConference(10, '123456', rec, recMerge).then( (data) => {
					...
					try{
						...
						console.log("----创建会议 发送邀请消息------start-------",me.members)
						wx.emedia.mgr.joinConference(confrId, '123456').then( (res) => {
							console.log('我已经加入会议了', res)
						//	me.sendInviteMsg(me.members, confrId, '123456') // 发送邀请消息
						}).catch(err=>{
							console.log('我已经加入会议了err:', JSON.stringify(err))
						})
						console.log("----创建会议并加入------end -------")
					}catch(e){
						//TODO handle the exception
						console.log("----创建会议并加入------ err-------",e)
					}
			...
			sendInviteMsg(members, confrId, password){
				...
				getSysUserInfo([wx.WebIM.conn.context.userId.split(CHAT_PREFIX)[1]]).then(res=>{
					 var users=res; 
					 try{
						users.forEach(user=>{
							if(user.uid==wx.WebIM.conn.context.userId.split(CHAT_PREFIX)[1] ){
								members&&members.forEach((value) => {
									let id = uni.WebIM.conn.getUniqueId();
									let msg = new wx.WebIM.message('txt', id);
											
									msg.set({
										msg:  user.nickname + ' 邀请您音视频通话',
			...
			hangup: function(){
				...
				uni.navigateBack({
					 url: "/pages/liveApp/chatroom/chatroom"
				});
			},
